...
export default function PharmacyOrdersPage() {
  ...
  useEffect(() => {
    ...
    // Socket setup with WebSocket priority
    ...
    socket.on('new-order', (data) => {
      toast.success('New Order Received!', {
        description: `Order ${data.orderId} for ₹${data.total}`,
        duration: 8000
      });
      loadOrders();
    });
  ...
  useEffect(() => {
    const fetchProfileAndJoinRoom = async () => {
      ...
      if (res.data && (res.data as any)._id) {
        const socket = io(SOCKET_URL);
        socket.emit('join-room', `pharmacy-${(res.data as any)._id}`);
      }
    };
  ...
  const handleStatusUpdate = async (orderId: string, newStatus: string) => {
    try {
      ...
      if (newStatus === 'accepted') {
        ...
        // Map UI 'preparing' to DB 'processing' 
        res = await ApiClient.updateOrderStatus(orderId, 'processing');
        toast.info("Order set to preparing. Finding nearest rider...");
      } else if (newStatus === 'out for delivery') {
        // Map UI 'out for delivery' to DB 'picked_up'
        res = await ApiClient.updateOrderStatus(orderId, 'picked_up');
      } else {
        ...
        <div className="md:col-span-3 space-y-6">
          <Card className="rounded-[2rem] border-gray-100 bg-white shadow-sm p-6 overflow-hidden">
            ...
            <div className="space-y-2">
              ...
              {['pending', 'accepted', 'processing', 'assigned', 'picked_up', 'delivered', 'cancelled'].map((status) => (
                <button
                  key={status}
                ...
                >
                  {status === 'processing' ? 'preparing' : status === 'picked_up' ? 'out for delivery' : status === 'assigned' ? 'dispatching' : status}
                </button>
              ))}
            </div>
          </Card>
                      ...
                      <div className="pt-8 mt-8 border-t border-gray-50 flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          ...
                          {order.status === 'accepted' && (
                            <Button
                              className="rounded-2xl h-11 bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase tracking-widest text-[9px] px-8 shadow-lg shadow-indigo-100"
